 ...
 */
public class OSCP {
	private static final String TAG = OSCP.class.getSimpleName();

	private static int timeoutConnection = 6000;
	private static boolean redirectsEnabled = true;
	private static List<Integer> acceptedHttpStatus = Arrays.asList(HttpURLConnection.HTTP_OK);
	...
	private static byte[] readHttpResponse(final String url, final HttpURLConnection httpClient) throws IOException {
		...
		InputStream content = httpClient.getInputStream();
